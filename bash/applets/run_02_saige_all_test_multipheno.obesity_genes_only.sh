...
# - "proteomics_v1" # The intended use of this pipeline, which only runs on a subset of genes, is on the proteomics data
readonly phenotype_group="proteomics_v1"
...
#for sex in {"both_sexes","female","male"}; do
  ...
  #for pheno_idx in {0..4}; do
    ...
    for chrom in {1..23}; do
      ...
      # Skip chr8, 10, 21, 22 because they have no significant genes
      elif [ ${chrom} -eq 8 ] || [ ${chrom} -eq 10 ] || [ ${chrom} -eq 21 ] || [ ${chrom} -eq 22 ]; then
        echo "Skipping chr${chrom}, no significant genes" && continue
      fi
      ...
      group_file="/nbaya/obesity/data/sig_obesity_genes_group_file/ukb_wes_450k.july.qced.brava.v6.obesity_genes_bhsigfdr0.01.tsv.gz" # NOTE: This is subset to only the genes which are Benjamini-Hochberg (FDR<=0.01) significant
