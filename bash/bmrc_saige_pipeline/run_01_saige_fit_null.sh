...
# [INPUT] Sparse GRM
readonly sparse_grm="data/00_set_up/ukb_array.wes_450k_qc_pass_${pop}.pruned_relatednessCutoff_0.05_5000_randomMarkersUsed.sparseGRM.mtx"
...
pheno_file="data/phenotypes/ukb_${phenotype_group}_phenos_and_covariates.${sex}.tsv"

# [OUTPUT] Output directory
out_dir="data/01_fit_null/${phenotype_group}/${pop}/${sex}" # directory for singularity container
out_dir_path="${WD}/${out_dir}" # path in BMRC
...
results_files="${out_dir}/${gwas_id}" # path for singularity container
results_files_path="${out_dir_path}/${gwas_id}" # path in BMRC

# Check if output file exists
if [[ ! -s ${results_files_path}.rda ]] || [[ ! -s ${results_files_path}.varianceRatio.txt ]]; then
  rm -f ${results_files_path}.*
  start_time=${SECONDS}
  singularity exec \
    --bind ${WD}:/mnt \
    ...
    ${trait_flags} \
    --outputPrefix="/mnt/${results_files}" \
