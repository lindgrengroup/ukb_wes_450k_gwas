...
if [ "$#" -ne 2 ]; then
    echo "Error: You must provide a phenotype group name and chromosome."
    echo "Usage: $0 <phenotype_group> <chrom>"
    exit 1
fi
...
echo "✅ Starting analysis for phenotype group=${PHENOTYPE_GROUP} chr${CHROM}"

# Read the phenotype list file line by line
while IFS= read -r pheno || [[ -n "$pheno" ]]; do
    echo "➡️ Processing Phenotype: ${pheno}"
    ...
    echo "  - Analyzing Chromosome ${CHROM}"
    ...
    ./plink \
      ...
      --freq case-control \
      --allow-no-sex \
      --out "${OUTPUT_DIR}/tmp-allele_freq_case_ctrl.${pheno}.chr${CHROM}" > /dev/null # Suppress verbose PLINK output
    
    # Clean up the temporary FAM file for the current phenotype
    rm "${TEMP_FAM_FILE}"

done < "$PHENO_LIST_PATH"
